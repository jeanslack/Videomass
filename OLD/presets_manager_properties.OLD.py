...
# Copyright © 2018-2019 Gianluca Pernigoto <jeanlucperni@gmail.com>
# license: GPL3
        ...
        if exclude:
            ...
            if not file_sources:
                wx.MessageBox("The selected profile is not suitable to convert "
                            "this file formats type: \n\n%s\n\n"
                            "...No other files queue to process." % (
                            '\n'.join(exclude)), 
                            "Unsuitable format - Videoamass", wx.OK, None)
                return (False)
            else:
                if wx.MessageBox("The selected profile is not suitable to "
                            "convert this file formats: \n\n%s\n\n"
                            "...Proceed with files queue?" % (
                            '\n'.join(exclude)), 
                            "Unsuitable format - Videoamass", wx.YES_NO |
                            wx.ICON_QUESTION, None) == wx.NO:
                            return (False)#Se L'utente risponde no
    return (file_sources)
