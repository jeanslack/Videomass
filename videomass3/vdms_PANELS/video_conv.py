...
class Video_Conv(wx.Panel):
    ...
    def __init__(self, parent, ffmpeg_link, ffplay_link, threads, 
        ...
        self.rdb_h264preset.SetToolTip(_("presets h.264"))
        self.rdb_h264preset.SetSelection(0)
        self.rdb_h264profile.SetToolTip(_("profile h.264"))
        self.rdb_h264profile.SetSelection(0)
        self.rdb_h264tune.SetToolTip(_("tune h.264"))
    ...
    def vidContainers(self, event):
        ...
        if vcodec[selected][0] == "-vcodec libx264":
            ...
            msg = (_("Tip: set a short time progress duration with `Duration` "
    ...
    def on_FiltersPreview(self, event):
        ...
        if not cmd_opt["Filters"]:
            wx.MessageBox(_("No filter enabled"), "Videomass2: Info", 
                          wx.ICON_INFORMATION)
            return
...
#------------------------------------------------------------------#
    def Addprof(self):
        ...
        prstdlg = presets_addnew.MemPresets(self, 'addprofile', full_pathname, 
                                            filename, list, 
                    _('Videomass2: Create a new profile on "%s" preset') % (
                                 name_preset))
        prstdlg.ShowModal()
