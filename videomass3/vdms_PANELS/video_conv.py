...
class Video_Conv(wx.Panel):
    ...
    def on_Automation(self, event):
        ...
        ####----------- Picture slideshow maker
            ...
            self.cmbx_Vaspect.Hide(), self.btn_audioAdd.Show()
            self.cmbx_Vrate.SetSelection(1), self.on_Vrate(self)
            cmd_opt["Shortest"][1] = '-shortest'
            if cmd_opt["Merging"]:
                self.notebook_1_pane_3.Enable()
    ...
    def on_ok(self):
        ...
        if self.rdb_auto.GetSelection() == 2:
            if not cmd_opt["Merging"]:
                wx.MessageBox(_('Missing audio track. Import an audio track '
                                'with the "Add audio track" control button.'), 
                                'Videomass', wx.ICON_INFORMATION)
                return
    ...
    def slideShow(self, file_sources, dest, logname):
        ...
        if len(file_sources) == 1: # loop with one pict and audio
            if not cmd_opt["Merging"]:
                wx.MessageBox(_('Missing audio track. Import an audio track '
                                'with the "Add audio track" control button.'), 
                            'Videomass', wx.ICON_INFORMATION, self)
        ...
        cmd_2 = [amount,
                ...
                '%s %s %s %s %s %s -pix_fmt yuv420p %s %s %s %s %s'
