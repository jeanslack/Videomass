...
class AV_Conv(wx.Panel):
    ...
    def __init__(self, parent, OS, iconplay, iconreset, iconresize, 
        ...
        grid_vp9panelprst.Add(self.cmb_vp9optimize, 0, 
                                                    wx.ALL |
                                                    wx.ALIGN_CENTER_HORIZONTAL|
                                                    wx.ALIGN_CENTER_VERTICAL, 5
                            )
        ...
        self.cmb_264optimize = wx.ComboBox(self.h264panel, wx.ID_ANY,  
                                           choices=[],
                                          size=(200,-1), style=wx.CB_DROPDOWN | 
                                                               wx.CB_READONLY
                                            )
        grid_h264panelprst.Add(self.cmb_264optimize, 0, wx.ALL |
                                                    wx.ALIGN_CENTER_HORIZONTAL|
                                                    wx.ALIGN_CENTER_VERTICAL, 5
                               )
    ...
    def on_Bitrate(self, event):
        ...
        cmd_opt["VideoBitrate"] = "-b:v %sk" % (self.spin_Vbrate.GetValue())
    ...
    def on_audioINstream(self, event):
        ...
        if sel == 'Auto':
            cmd_opt["AudioInMap"] = ['','']
            if not self.rdb_a.GetStringSelection() == 'No Audio':
    ...
    def video_ebu_2pass(self, f_src, destin, countmax, logname):
        ...
        if self.cmb_Vcod.GetValue() == "Copy":
            ...
            cmd_1 = (f'{cmd_opt["VideoCodec"]} {cmd_opt["VideoBitrate"]} '
                     ...
                     f'{opt1} -sn -filter:a: '
                     f'{loudfilter} -f {muxers[cmd_opt["OutputFormat"]]}'
                     )
